...
stateDict  = [ [
    {"utility": 0, "bestAction":"none", "reward": ( lambda : -0.04 if my_array[i][j] ==0 else (1 if my_array[i][j] == 1 else (-1 if my_array[i][j] ==2 else 0)))()   } 
        for j in range(4) ] for i in range(3)]

temp_dict = [[( lambda : -0.04 if my_array[i][j] ==0 else (1 if my_array[i][j] == 1 else (-1 if my_array[i][j] ==2 else 0)))() for j in range(4)] for i in range(3)]
...
def value_iterate():
    ...
    while (count != 15) :
        ...
        print(allQValues)
        all_actions_taken.append(action_taken)
